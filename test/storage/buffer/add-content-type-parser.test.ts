...
t.test('BufferStorage - addContentTypeParser', function (t) {
  t.plan(3)
  ...
  t.test('single file', async function (t) {
    ...
    t.same(json.body.file, { name: 'hello_world.txt', value: { type: 'Buffer', data: Buffer.from('helloworld').map(Number) } })
